...
class CliJavaModuleFinder(
    ...
    override fun findModule(name: String): JavaModule? =
        (if (useLastJdkApi)
            modulesRoot?.findChild(name)?.let(this::findSystemModule) else ctSymModules[name]?.let { findSystemModule(it, true) })
            ?: userModules[name]
    ...
    private fun collectModuleRoots(release: Int): Map<String, VirtualFile> {
        ...
        if (isJDK12OrLater) {
            listFoldersForRelease(release).forEach { modulesRoot ->
